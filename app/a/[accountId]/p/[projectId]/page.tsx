...
import AddDependencyButton from '@/app/components/AddDependencyButton';
import { executeTaskAction } from '@/app/actions/taskActions';

export default async function AgencyProjectPage({ params }: { params: Promise<{ accountId: string; projectId: string }> }) {
  ...
  if (!project || project.accountId !== resolvedParams.accountId) notFound();
  ...
  return (
    ...
    >
      <div style={{ padding: '0 24px 12px', display: 'flex', justifyContent: 'flex-end' }}>
        <AddDependencyButton projectId={project.id} />
      </div>
      ...
      {tasks.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 24px' }}>
          ...
          <p style={{ color: 'var(--faint)', fontSize: '14px', maxWidth: '400px', margin: '0 auto 24px' }}>
            Add your first dependency to start tracking what this client needs to deliver.
          </p>
          <AddDependencyButton projectId={project.id} />
        </div>
      ) : (
